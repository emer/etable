...
type Splits struct {
	...
	Levels []string `desc:"levels of indexes used to organize the splits -- each split contains the full outer product across these index levels.  for example, if the split was generated by grouping over column values, then these are the column names in order of grouping.  the splits are not automatically sorted hierarchically by these levels but e.g., the GroupBy method produces that result -- use the Sort methods to explicitly sort."`
	...
	Values [][]string `desc:"the values of the index levels associated with each split.  The outer dimension is the same length as Splits, and the inner dimension is the levels."`
	...
	Aggs []*SplitAgg `desc:"aggregate results, one for each aggregation operation performed -- split-level data is contained within each SplitAgg struct -- deleting a split removes these aggs but adding new splits just invalidates all existing aggs (they are automatically deleted)."`

	// [view: -] current Less function used in sorting
	lessFunc SplitsLessFunc `copy:"-" view:"-" xml:"-" json:"-" desc:"current Less function used in sorting"`
}
