...
func SaveCSV(tsr Tensor, filename string, delim rune) error {
...
func OpenCSV(tsr Tensor, filename string, delim rune) error {
...
func ReadCSV(tsr Tensor, r io.Reader, delim rune) error {
	...
	for ri := 0; ri < rows; ri++ {
		for ci := 0; ci < cols; ci++ {
			str := rec[ri][ci]
			if str == "" {
				tsr.SetNull1D(idx, true) // empty = missing
			} else {
